...
* `all_detect_easy_V3.py`：简单图形测量：正方形、三角形、圆形三合一，可测距，基础题快速解决。
* `detect_multi_shapes_V12_pi.py`：多图形检测，可应对分离或融合（重叠）图形，精度高，效果拔群。
* `detect_turn.py`：检测斜放置场景精度高，对旋转阈值无要求，覆盖比赛旋转范围。

### 5.2 工具模块（tool/ 文件夹）

#### (1) pxtool.py

* 功能：计算物体的 **外框像素长度**，作为距离测量的校准依据。
* 使用方法：
  将物体放在标准点位（100, 105, 110, … 200 cm，尽量平均分布），运行 `pxtool.py` 输出像素长度，结果用于自行填写校准表。
* 示例校准表：
  ...
  例如：761.0 像素对应实际 200 cm。
* 作用：该表在所有具备测距功能的代码中，均作为核心 **校准数据** 使用。

#### (2) gpio\_test.py

* 功能：测试树莓派 GPIO 按键输入输出，验证硬件连接是否正确。
* 使用方法：运行脚本，按下按钮即可在终端实时显示 GPIO 引脚的状态变化。
* 示例代码：
  ...
  ```
* 用途：方便调试，快速确认按钮/传感器是否工作正常，为主程序提供硬件保障。
